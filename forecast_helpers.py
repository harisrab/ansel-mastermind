...
def generate_forecast_on_all(df, bucketing_duration: str, organization_id: str):
    assert bucketing_duration in ['daily', 'weekly', 'monthly',
                                  'quarterly'], "Invalid bucketing_duration. Must be one of ['daily', 'weekly', 'monthly', 'quarterly']"

    # Get the list of products
    products = [{"product_title": title}
                for title in list(set(df["product_title"].tolist()))]
...
def extend_datetime_arr(date_strings):
    ...
    dates = [datetime.strptime(s, '%Y-%m-%d') if s !=
             'nan' else 'nan' for s in date_strings]

    # Calculate the difference between consecutive dates
    diffs = [(dates[i+1] - dates[i]).days for i in range(len(dates)-2)
    ...
    date_strings = [date.strftime(
        '%Y-%m-%d') if date != 'nan' else 'nan' for date in dates]

    return date_strings


def triple_exp_smooth_add(demand, day_list, slen=12, extra_periods=1, params={"alpha": 0.4, "beta": 0.4, "phi": 0.9, "gamma": 0.3}):
...
def source_demand_bucketing(df, duration="weekly"):
    ...
    if duration == "daily":
        demand_df = df.sort_values(['product_title', 'day']).resample(
            'D', on='day')['ordered_item_quantity'].sum().reset_index()

    elif duration == "weekly":
        demand_df = df.sort_values(['product_title', 'day']).resample(
            'W', on='day')['ordered_item_quantity'].sum().reset_index()

    elif duration == "monthly":
        demand_df = df.sort_values(['product_title', 'day']).resample(
            'M', on='day')['ordered_item_quantity'].sum().reset_index()

    elif duration == "quarterly":
        demand_df = df.sort_values(['product_title', 'day']).resample(
            'Q', on='day')['ordered_item_quantity'].sum().reset_index()
    ...
    day_list = [timestamp.strftime('%Y-%m-%d')
                for timestamp in demand_df['day'].to_list()]
